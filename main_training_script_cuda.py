...
# *** NEW: Create directories for saving cell-by-cell error grids ***
os.makedirs('saved_arrays/x_component', exist_ok=True)
os.makedirs('saved_arrays/y_component', exist_ok=True)
print("Cell-by-cell error grids will be saved in the 'saved_arrays' directory.")
...
for ep in range(ep_num):
    ...
    # *** NEW: Initialize tensors to store the sum of error grids for the epoch ***
    total_error_grid_x = torch.zeros((1, 1, n, n), device=f'cuda:{gpu_id}')
    total_error_grid_y = torch.zeros((1, 1, n, n), device=f'cuda:{gpu_id}')
    
    for img_in, img_out_x, img_out_y in val_loader: # MODIFIED: was train_loader, corrected to val_loader
        img_in, img_out_y, img_out_x = Variable(img_in.cuda(gpu_id)),  Variable(img_out_y.cuda(gpu_id)),Variable(img_out_x.cuda(gpu_id))
        recx = netx.forward(img_in)
        recy = nety.forward(img_in)
        ...
        # =================================================================================
        # MODIFIED: Calculate and accumulate cell-by-cell NORMALIZED error for the batch.
        # This new calculation preserves the sign to show over/under prediction.
        # =================================================================================
        with torch.no_grad():
            # --- X-Component Normalized Error ---
            
            # 1. Calculate the raw error (Truth - Prediction) for the entire batch.
            #    We do NOT use abs() because we want to keep the sign.
            raw_error_batch_x = img_out_x - recx
            
            # 2. Find the max and min for EACH ground truth image in the batch.
            B = img_out_x.shape[0] # Batch size
            flat_truth_x = img_out_x.view(B, -1)
            truth_max_x = torch.max(flat_truth_x, dim=1, keepdim=True)[0].view(B, 1, 1, 1)
            truth_min_x = torch.min(flat_truth_x, dim=1, keepdim=True)[0].view(B, 1, 1, 1)

            # 3. Calculate the range. If range is near-zero, set it to 1.0
            #    to avoid division-by-zero errors and leave the raw error unscaled.
            truth_range_x = truth_max_x - truth_min_x
            truth_range_x[truth_range_x < 1e-6] = 1.0

            # 4. Normalize the raw error using the range of each corresponding truth image.
            normalized_error_batch_x = raw_error_batch_x / truth_range_x

            # 5. Sum the normalized error grids across the batch and add to the epoch total.
            total_error_grid_x += torch.sum(normalized_error_batch_x, dim=0, keepdim=True)

            # --- Y-Component Normalized Error (repeating the same logic) ---
            raw_error_batch_y = img_out_y - recy
            
            flat_truth_y = img_out_y.view(B, -1)
            truth_max_y = torch.max(flat_truth_y, dim=1, keepdim=True)[0].view(B, 1, 1, 1)
            truth_min_y = torch.min(flat_truth_y, dim=1, keepdim=True)[0].view(B, 1, 1, 1)
            
            truth_range_y = truth_max_y - truth_min_y
            truth_range_y[truth_range_y < 1e-6] = 1.0
            
            normalized_error_batch_y = raw_error_batch_y / truth_range_y

            total_error_grid_y += torch.sum(normalized_error_batch_y, dim=0, keepdim=True)
    ...
    # *** NEW: Average, convert, and save the error grids for the completed epoch ***
    with torch.no_grad():
        # Calculate the average error grid for the epoch
        avg_error_grid_x = total_error_grid_x / val_sample_num
        avg_error_grid_y = total_error_grid_y / val_sample_num
        
        # Convert to numpy and remove unnecessary batch/channel dimensions
        error_numpy_x = avg_error_grid_x.squeeze().cpu().numpy()
        error_numpy_y = avg_error_grid_y.squeeze().cpu().numpy()
        
        # Define file paths using the specified naming convention
        path_x = f'saved_arrays/x_component/error_gridx_ep{ep}.npy'
        path_y = f'saved_arrays/y_component/error_gridy_ep{ep}.npy'
        
        # Save the numpy arrays
        np.save(path_x, error_numpy_x)
        np.save(path_y, error_numpy_y)
    ...
    plt.subplot(2, 5, 4)
    # MODIFICATION FOR X-COMPONENT ERROR MAP
    im = plt.imshow( Img , cmap='jet', vmin=-0.5, vmax=0.7)
    ...
    Img = (img[..., 0] - img1[..., 0]) / (img[..., 0].max() - img[..., 0].min())                                       # This normalization scheme is used. img1 is predictions, img is the truth, Img is the normalized error
    plt.subplot(2, 5, 9)
    # MODIFICATION FOR Y-COMPONENT ERROR MAP
    im = plt.imshow( Img , cmap='jet', vmin=-0.4, vmax=1.4)
    plt.title('Error normalized')
    plt.colorbar(im, fraction=0.046, pad=0.04)
    plt.axis('off')
